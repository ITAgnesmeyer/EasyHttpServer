...
namespace EasyHttpServer
{
    public class HttpServer : IDisposable
    {
        ...
        private void TaskAction(object o)
        {
            ...
            {
                foreach (var url in server.ListenerUrls)
                {
                    try
                    {
                ...
                try
                {
                    ...
                    foreach (var prefixUrl in server.CurrentListener.Prefixes)
                    {
                ...
                try
                {
                    while (LoopEnable)
                    {
                        var context = server.CurrentListener.GetContext();
                        var absPath = context.Request.Url.LocalPath;
                        var found = CheckUrlAndGetPath(absPath, server.RootPath, server.IndexFiles, out var path);
                        ...
                        using (var response = context.Response)
                        {
                            ...
                            var args = new ServerPreRequestHandlerEventArgs(context.Request);
                            try
                            {
                                try
                                {
                            ...
                            if (args.Handled)
                            {
                                ...
                                var bytes = Encoding.UTF8.GetBytes(args.ContentToSend);
            ...
            string path)
        {
            if (!found)
            {
                var message = string.Format(server.NotFoundMessage, absPath);
            ...
            if (File.Exists(path))
            {
                ...
                {
                    var message = string.Format(server.InternalServerErrorMessage, e.Message);
            ...
            {
                var message = string.Format(server.BadRequestMessage, absPath);
